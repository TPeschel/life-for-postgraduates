...
tbl.psy.allerg <- tbl[ !is.na( tbl$C_DISEASE_TX_PSY )
                       & !is.na( tbl$C_DISEASE_TX_ALLERG ), ]
...
glm.male   <- glm( data = tbl.psy.allerg[SEX == "male",], 
                   score.psy ~ score.allerg, 
                   family = binomial( "logit" ) )
glm.female <- glm( data = tbl.psy.allerg[ SEX == "female", ], 
                   score.psy ~ score.allerg, 
                   family = binomial( "logit" ) )

glm.allg.   <- glm( data = tbl.psy.allerg, 
                   score.psy ~ score.allerg, 
                   family = binomial( "logit" ) )
coef (glm.allg.)
...
ggplot(tbl.psy.allerg, aes(x=score.allerg, y=score.psy)) + geom_point(
