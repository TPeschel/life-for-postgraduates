...
# Die Funktion "filter.for.ones" filtert alle Zeilen, die bei irgendeiner Krankheit oder Medikation eine 1 stehen haben, aus.
...
# df <- filter.for.ones( df, diseases )
# df <- filter.for.ones( df, medik )

# Weils so lange dauert, ist es wohl besser diseases und medik in einem Rutsch zu erledigen.
df <- filter.for.ones( df, c( medik, diseases ) )
...
library( WriteXLS ) 
WriteXLS( x = neue.Tabelle , ExcelFileName = "AktuelleTabelle190517excel.xlsx" )

library( openxlsx )
write.xlsx( x = neue.Tabelle, file = "AktuelleTabelle190517excel2.xlsx" )
